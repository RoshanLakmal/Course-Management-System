...
public class Course implements Comparable<Course>, Serializable {
	...
	public Course(String course_name, String instructor, int course_id, int section, String location, int max_registered) {
	...
	public int search(Student student) {
		for (int i = 0; i < registered_students.size(); i++) {
			if (student.get_student_id() == registered_students.get(i).get_student_id()) {
				return i;
			}
		}
	...
	// an overriden compareTo method used by Collection sort to organize by course registry size

	public int compareTo(Course course_compare) {
		Integer a = new Integer(course_compare.get_registered());
		Integer b = new Integer(this.registered);
