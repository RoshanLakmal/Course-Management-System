...
public class Student extends User implements Serializable, SInterface {
	...
	public Student(String username, String password, String first_name, String last_name, int student_id) {
	...
	public int search(Course course) {
		for (int i = 0; i < registered_courses.size(); i++) {
			if ((course.get_course_id() == registered_courses.get(i).get_course_id()) && (course.get_section() == registered_courses.get(i).get_section())) {
				return i;
			}
		}
